...
class RewardService {
    ...
    private async sendRewardToken(smartWalletAddress: string, _soulMetadata: SoulMetaData, _ownership: string[]) {
        try {

            const requestBody = { toAddress: smartWalletAddress, amount: "100"}
            await engine.erc20.transfer(Arbitrum.chainId.toString(), BEATS_TOKEN, ENGINE_ADMIN_WALLET_ADDRESS, requestBody);
