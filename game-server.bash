...
sudo apt update -y
sudo apt upgrade -y
sudo apt install -y docker.io nginx unzip git curl certbot python3-certbot-nginx openssl libssl-dev tcl tcl-dev
...
cat <<EOF | sudo tee /etc/nginx/sites-available/docker.gmetarave.com
...
server {
    listen 80;
    server_name docker.gmetarave.com;

    if ($host = docker.gmetarave.com) {
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/docker.gmetarave.com/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/docker.gmetarave.com/privkey.pem; # managed by Certbot
    include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot

    # Location for /memgraph
    location /memgraph {
...
# Enable the site
sudo ln -s /etc/nginx/sites-available/docker.gmetarave.com /etc/nginx/sites-enabled/
...
echo "Requesting SSL certificate for docker.gmetarave.com..."
sudo certbot --nginx -d docker.gmetarave.com --non-interactive --agree-tos -m paulanthonyarriola@gmail.com
...
# # Step 24: Add DOTENV_KEY to environment and source it
# echo 'export DOTENV_KEY=dotenv://:key_ff4cfd5e35acf95c9141b6b5dc7c366ac33c4553b875672a0561ab40353011c4@dotenv.org/vault/.env.vault?environment=production' >> ~/.bashrc
# source ~/.bashrc
